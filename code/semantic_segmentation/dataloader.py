...
class ImageFolder(data.Dataset):
	def __init__(self, root, image_size = 224, mode='train', augmentation_prob = 0.4):
		...
		print("# of {} samples: {}".format(self.mode, len(self.image_paths))) 
...
def get_loader(image_path, image_size, batch_size, num_workers=2, mode='train',augmentation_prob=0.4, isshuffle=True):
	"""Builds and returns Dataloader."""
	
	dataset = ImageFolder(
		root = image_path, image_size = image_size, 
		mode = mode, augmentation_prob = augmentation_prob)

	data_loader = data.DataLoader(
		dataset = dataset, batch_size = batch_size,
		shuffle = isshuffle, num_workers = num_workers)
